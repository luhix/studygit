		...
		<article class="cl pd-20">
			...
			<div class="cl pd-5 bg-1 bk-gray mt-20">
				<span class="l">
					...
					<a href="javascript:;" onclick="admin_add('添加管理员','<?php echo url('user/adminAdd'); ?>','800px','500px')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加管理员</a>
				 </span>
				...
				<tbody>
					...
					<tr class="text-c">
						...
						<td class="td-status">
							...
							<?php else: ?>
								<span class="label label-default radius"><?php echo $vo['status']; ?></span>
							<?php endif; ?>
						</td>
						<td class="td-manage">
							<!--只允许admin有权启用或停用, 切换图标-->
							<?php if(\think\Session::get('user_info.name') == 'admin'): if($vo['status'] == '已启用'): ?>
									<a style="text-decoration:none" onClick="admin_stop(this,<?php echo $vo['user_id']; ?>)" href="javascript:;" title="停用"><i class="Hui-iconfont">&#xe631;</i></a>
								<?php else: ?>
									<a style="text-decoration:none" onClick="admin_start(this,<?php echo $vo['user_id']; ?>)" href="javascript:;" title="启用"><i class="Hui-iconfont">&#xe615;</i></a>
								<?php endif; endif; ?>

							<a title="编辑" href="javascript:;" onclick="admin_edit('编辑管理员','adminEdit?id=<?php echo $vo['user_id']; ?>',800,500)" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i>
							</a>

							<!-- 只有admin有权利删除 -->
							<?php if(\think\Session::get('user_info.name') == 'admin'): ?>
							<a title="删除" href="javascript:;" onclick="admin_del(this,<?php echo $vo['user_id']; ?>)" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a>
							<?php endif; ?>
						</td>
					</tr>
...
function unDelete() {
    ...
    layer.confirm('确认要恢复吗？', function (res) {
        $.get("<?php echo url('user/unDelete'); ?>");

        layer.msg('已恢复!', {icon: 1, time: 2000}); //消息弹层1秒后消失
